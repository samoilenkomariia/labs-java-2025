...
public class ShapeControllerTest {
    ...
    @Test
    void testSaveAndReadData_Success() {
        File tempFile = tempDir.resolve("shapes.txt").toFile();
        String filePath = tempFile.getAbsolutePath();
        controller.saveDataToFile(filePath);

        assert view.printMessageCount == 1;
        assert view.lastMessagePassed.equals("Saved shapes to file");
        assert tempFile.exists();
        assert tempFile.length() > 0;

        view.reset();
        ShapeModel newModel = new ShapeModel(new Shape[]{});
        ShapeController newController = new ShapeController(view, newModel);

        newController.readDataFromFile(filePath);
        assert view.printMessageCount == 1;
        assert view.lastMessagePassed.equals("Shapes updated successfully from file");

        assert newModel.getShapes().length == model.getShapes().length;
        assert newModel.calcTotalArea() == model.calcTotalArea();
    }

    @Test
    void testReadDataFromFile_FileNotFound() {
        String filePath = tempDir.resolve("non_existent_file.txt").toString();
        controller.readDataFromFile(filePath);

        assert view.printMessageCount == 1;
        assert view.lastMessagePassed.startsWith("File %s not found".formatted(filePath));
    }

    @Test
    void testReadDataFromFile_CorruptedFile() throws IOException {
        File badFile = tempDir.resolve("bad_file.dat").toFile();
        try (FileWriter writer = new FileWriter(badFile)) {
            writer.write("this is not valid serialized Java object data");
        }
        String filePath = badFile.getAbsolutePath();
        controller.readDataFromFile(filePath);

        assert view.printMessageCount == 1;
        assert view.lastMessagePassed.startsWith("Error reading file");
    }

    @Test
    void testSaveDataToFile_WriteError() {
        String badPath = tempDir.toFile().getAbsolutePath();
        controller.saveDataToFile(badPath);

        assert view.printMessageCount == 1;
        assert view.lastMessagePassed.startsWith("Error writing to file %s".formatted(badPath));
    }
